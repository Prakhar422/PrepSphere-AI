...
const QuizHistory = () => {
  ...
  const handleDeleteAttempt = async () => {
    ...
    setIsDeleting(true);
    try {
      const response = await api.delete(`/aptitude/history/${deleteTarget}`);
      if (response.data && response.data.success) {
        setHistoryList(prev => prev.filter(item => item.attemptId !== deleteTarget));
        setDeleteTarget(null);
        setSuccessToast("Quiz attempt deleted successfully.");
        setTimeout(() => setSuccessToast(""), 4000);
      } else {
        throw new Error(response.data?.message || "Failed to delete attempt.");
      }
    } catch (err) {
      console.error("Error deleting quiz attempt:", err);
      alert(err.response?.data?.message || err.message || "An error occurred while deleting the quiz attempt.");
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const filteredHistory = historyList.filter(item => {
    ...
    if (selectedDifficulty === "All") return categoryMatch;
    return categoryMatch && item.difficulty?.toLowerCase() === selectedDifficulty.toLowerCase();
  });
  ...
  // Statistics calculation from backend history data
  const calculateStats = () => {
    const totalAttempts = historyList.length;
    if (totalAttempts === 0) {
      return { totalAttempts: 0, avgAccuracy: 0, highestScore: 0, bestAccuracy: 0 };
    }
    const sumAccuracy = historyList.reduce((acc, item) => acc + (item.accuracy || 0), 0);
    const avgAccuracy = Math.round(sumAccuracy / totalAttempts);
    const highestScore = Math.max(...historyList.map(item => item.score || 0));
    const bestAccuracy = Math.max(...historyList.map(item => item.accuracy || 0));
    return { totalAttempts, avgAccuracy, highestScore, bestAccuracy };
  };
            ...
            <AnimatePresence>
              {successToast && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -20 }}
                  className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 px-4 py-3 rounded-xl flex items-center gap-2 text-sm text-left shadow-lg"
                >
            ...
            <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6 text-left">
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-300 leading-relaxed font-light">
                  Review your previous aptitude assessments and monitor your progress.
                </p>
              </div>
            ...
            {!loading && !error && historyList.length > 0 && (
              <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 text-left relative overflow-hidden group hover:border-indigo-500/25 transition-all">
                  ...
                  <div className="mt-2 text-xs text-slate-500">Completed quizzes</div>
                </div>
                ...
                <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 text-left relative overflow-hidden group hover:border-purple-500/25 transition-all">
                  ...
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Average Accuracy</span>
                  ...
                  <div className="mt-2 text-xs text-slate-500">Across all completed runs</div>
                </div>
                ...
                <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 text-left relative overflow-hidden group hover:border-cyan-500/25 transition-all">
                  ...
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Highest Score</span>
                    <div className="p-1.5 rounded-lg bg-cyan-500/10 text-cyan-400">
                      <CheckCircle2 className="w-4 h-4" />
                    </div>
                  </div>
                  <div className="mt-4">
                    <span className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400 font-mono">
                      {stats.highestScore} <span className="text-sm text-slate-500">/ 10</span>
                    </span>
                  </div>
                  <div className="mt-2 text-xs text-slate-500 font-semibold text-cyan-400">Correct answers peak</div>
                </div>
                ...
                <div className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 text-left relative overflow-hidden group hover:border-pink-500/25 transition-all">
                  ...
                  <div className="flex justify-between items-start">
                    <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Best Accuracy</span>
                  ...
                  <div className="mt-2 text-xs text-slate-500">Highest single attempt rate</div>
                </div>
              </section>
            )}
            ...
            {!loading && !error && historyList.length > 0 && (
              <section className="flex flex-col sm:flex-row gap-4 items-center justify-between">
                ...
                <div className="flex flex-wrap gap-2 justify-start w-full sm:w-auto">
                  {["All", "Easy", "Medium", "Hard", "Adaptive"].map((diff) => (
                    <button
                      key={diff}
                      onClick={() => {
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
                  <span className="text-xs text-slate-500 font-medium font-inter">Sort By:</span>
                  <select
                    value={sortBy}
                    onChange={(e) => {
            ...
            {loading && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 relative overflow-hidden animate-pulse h-[220px] flex flex-col justify-between"
                  >
            ...
            {!loading && !error && sortedHistory.length === 0 && (
              <div className="flex flex-col items-center justify-center p-16 bg-white/[0.02] border border-white/10 rounded-3xl backdrop-blur-xl text-center max-w-lg mx-auto space-y-5">
                ...
                <div className="space-y-1.5">
                  <h3 className="text-lg font-bold text-white">No Quiz History Yet</h3>
                  <p className="text-sm text-slate-400 leading-relaxed font-light max-w-xs mx-auto">
                    Complete your first aptitude quiz to start building your performance history.
                  </p>
                </div>
                <button
                  onClick={() => navigate("/aptitude-practice")}
                      ...
                      <div className="space-y-4 relative z-10">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            <h4 className="text-sm font-bold text-white truncate max-w-[200px]" title={item.category}>
                              {item.category}
                            </h4>
                            <span className="text-[11px] text-slate-500 font-light font-mono block mt-1">
                        ...
                        {/* Summary details */}
                        <div className="grid grid-cols-2 gap-y-3 gap-x-2 pt-2 border-t border-white/5 text-xs text-slate-300">
                          <div>
                            <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider">Score</span>
                            <span className="text-sm font-bold text-indigo-400 font-mono">{item.score} / 10</span>
                          </div>
                          <div>
                            <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider">Accuracy</span>
                            <span className="text-sm font-bold text-emerald-400 font-mono">{item.accuracy}%</span>
                          </div>
                          <div>
                            <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider">Correct / Wrong</span>
                            <span className="text-slate-400 font-mono text-[11px]">
                              {item.correctAnswers} C | {item.wrongAnswers} W | {item.skippedQuestions} S
                            </span>
                          </div>
                          <div>
                            <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider">Time Taken</span>
                            <span className="text-slate-400 font-mono text-[11px]">
                        ...
                        <div className="flex items-center gap-2 pt-4 border-t border-white/5 mt-4">
                          <button
                            onClick={() => navigate(`/aptitude/history/${item.attemptId}`)}
                            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-xs font-bold transition-all cursor-pointer shadow-md shadow-indigo-500/15 hover:scale-[1.01]"
                          >
                            View Report
                          </button>
                          
                          <button
                            onClick={() => setDeleteTarget(item.attemptId)}
                            className="p-2 rounded-xl bg-red-500/5 hover:bg-red-500/15 border border-red-500/10 text-red-400 hover:text-red-300 transition-all cursor-pointer focus:outline-none"
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-white">Delete this quiz attempt?</h3>
                    <p className="text-sm text-slate-400 font-light mt-1.5 leading-relaxed">
                      Are you sure you want to permanently delete this quiz attempt record? This action cannot be undone, and your stats will be recalculated.
                    </p>
                  </div>
                </div>
