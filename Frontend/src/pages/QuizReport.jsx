  ...
  const fetchReportDetails = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        throw new Error(response.data?.message || "Failed to load quiz report details.");
      }
    } catch (err) {
      console.error("Error loading quiz report:", err);
      setError(err.response?.data?.message || err.message || "An error occurred while loading quiz report.");
    } finally {
      setLoading(false);
    }
  };
          ...
          <header className="sticky top-0 z-30 bg-[#050B1F]/70 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 hover:bg-white/10 text-slate-300 hover:text-white text-xs font-semibold transition-all cursor-pointer"
              >
            ...
            <div className="flex items-center space-x-6 shrink-0 pl-4">
              <div className="flex items-center space-x-3 border-white/5 pl-6">
            ...
            {loading && (
              <div className="space-y-6 animate-pulse text-left">
                ...
                <div className="space-y-4">
                  {[...Array(3)].map((_, i) => (
                    <div key={i} className="bg-white/[0.02] border border-white/10 rounded-2xl p-5 h-[140px]" />
                  ))}
                </div>
              </div>
            )}
                ...
                <div className="relative rounded-3xl border border-white/10 bg-slate-950/20 backdrop-blur-xl p-6 sm:p-8 overflow-hidden">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="space-y-2">
                      ...
                      <h2 className="text-2xl font-black text-white">Assessment Report Summary</h2>
                      
                      <p className="text-xs text-slate-400 font-light font-mono">
                        Submitted On {new Date(summary.submittedDate).toLocaleDateString("en-US", {
                          month: "long",
                          day: "numeric",
                          year: "numeric",
                          hour: "2-digit",
                    ...
                    <div className="flex items-center gap-4 bg-white/[0.01] border border-white/5 rounded-2xl p-4 shrink-0 font-mono">
                      <div className="text-center px-2">
                        <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Score</span>
                        <span className="text-2xl font-black text-indigo-400">{summary.score} / 10</span>
                      </div>
                      <div className="h-8 w-[1px] bg-white/10" />
                      <div className="text-center px-2">
                        <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Accuracy</span>
                        <span className="text-2xl font-black text-emerald-400">{summary.accuracy}%</span>
                      </div>
                      <div className="h-8 w-[1px] bg-white/10" />
                      <div className="text-center px-2">
                        <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Duration</span>
                        <span className="text-sm font-bold text-slate-300">
                          {Math.floor(summary.timeTaken / 60)}m {summary.timeTaken % 60}s
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-6 pt-6 border-t border-white/5 text-center text-xs text-slate-400 font-mono">
                    <div>
                      <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Correct</span>
                      <span className="text-sm font-bold text-emerald-400">{summary.correctAnswers} / 10</span>
                    </div>
                    <div className="border-l border-white/5">
                      <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Incorrect</span>
                      <span className="text-sm font-bold text-red-400">{summary.wrongAnswers} / 10</span>
                    </div>
                    <div className="border-l border-white/5">
                      <span className="block text-[9px] text-slate-500 font-bold uppercase tracking-wider mb-1">Skipped</span>
                      <span className="text-sm font-bold text-amber-400">{summary.skippedQuestions} / 10</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-5">
                    {questions.map((q, idx) => {
                      ...
                      const isSkipped = q.userSelectedOption === null || q.userSelectedOption === "";
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className={`absolute left-0 top-0 bottom-0 w-[4px] ${
                            isSkipped ? "bg-amber-500/50" : isCorrect ? "bg-emerald-500/50" : "bg-red-500/50"
                          }`} />
                          ...
                          <div className="space-y-4 pl-2">
                            ...
                            <div className="grid grid-cols-1 gap-3 pt-2">
                              {(q.options || []).map((opt) => {
                                ...
                                const isUserSelected = opt.key === q.userSelectedOption;

                                let optionStyle = "bg-slate-950/20 border-white/5 text-slate-300";
                                let badgeStyle = "bg-white/5 text-slate-400";

                                if (isUserSelected && isCorrectOption) {
                                  // User selected correct answer
                                  optionStyle = "bg-blue-500/10 border-blue-500/50 text-blue-300 shadow-[0_0_15px_rgba(59,130,246,0.1)]";
                                  ...
                                  optionStyle = "bg-red-500/10 border-red-500/50 text-red-300 shadow-[0_0_15px_rgba(239,68,68,0.1)]";
                                ...
                                {isExpanded && (
                                  ...
                                  >
                                    <div className="p-4 bg-slate-950/40 border border-white/5 rounded-xl text-xs text-slate-300 leading-relaxed font-light">
                                      <p className="font-semibold text-slate-200 mb-1.5">Solution Approach &amp; Concept Details:</p>
                                      <p className="font-mono whitespace-pre-line text-indigo-200">{q.explanation || "No explanation details compiled."}</p>
                                    </div>
                                  </motion.div>
                                )}
                              </AnimatePresence>
