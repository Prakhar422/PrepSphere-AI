...
const ProtectedRoute = ({ children }) => {
  ...
  if (loading) {
    return (
      <div className="relative min-h-screen bg-[#050B1F] flex items-center justify-center font-inter overflow-hidden">
        ...
        <div className="flex flex-col items-center space-y-4 z-10 text-center">
          ...
          <div>
            <h3 className="text-sm font-bold text-white tracking-wider uppercase">Authenticating</h3>
            <p className="text-[10px] text-slate-500 font-light mt-0.5">Initializing secure console...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (!user) {
    // Redirect to login but keep current location in state for redirecting after login
